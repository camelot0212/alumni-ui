...
import { Home, Calendar, Users, UserCircle, PlusSquare, LogOut } from 'lucide-react';
  ...
  return (
    <div className="hidden md:flex flex-col w-64 h-screen fixed left-0 top-0 bg-white border-r border-gray-200 z-20">
      ...
      <div className="p-4 border-t border-gray-100">
        <div onClick={onLogout} className="flex items-center gap-3 p-2 rounded-lg hover:bg-red-50 cursor-pointer transition-colors group">
          <img src={USERS.me.avatar} alt="Me" className="w-9 h-9 rounded-full" />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-900 truncate group-hover:text-red-700">{USERS.me.name}</p>
            <p className="text-xs text-gray-500 truncate group-hover:text-red-400">Sign Out</p>
          </div>
